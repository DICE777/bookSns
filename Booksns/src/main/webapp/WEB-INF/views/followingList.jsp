...
<div class="defaultMain ui bottom attached padded">
	<div class="ui main text container" align="center">		     
		...
		<c:if test="${following != '[]'}">
			<c:forEach var="following" items="${following}" varStatus="fNum">
				<div class="ui card">
					<div class="blurring dimmable image">
						<c:if test="${following.saveFile ne null}">
							<img src="http://localhost:8888/sns/resources/images/FileRepo/${following.saveFile}">
						</c:if>
						<c:if test="${following.saveFile eq null}">
							<img src="http://localhost:8888/sns/resources/images/defaultAvatar.png">
						</c:if>
					</div>
					<div class="content">
						<a class="header left floated" href="/sns/profileForm?userId=${following.userId}">${following.userId}</a>
							<a href="unfollow?followId=${following.userId}">
